...
public class PlayerMovementMobile : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        this.transform.rotation = Quaternion.Slerp(transform.rotation, q, Time.deltaTime  * m_Game.TimeManager.timeScale* m_tankController.TracksController.GetTrackRotationSpeed()/100);

        
        m_xForce = m_tankController.TracksController.GetTrackSpeed() * 
                   Time.deltaTime*
                   power * 
        ...
        m_yForce = m_tankController.TracksController.GetTrackSpeed()
